...
2. Execute a migration para estruturar os dados:
   ```bash
   php migration.php
   ```
...
Esse script detectará as configurações de `database.php`. No caso do MySQL, criará o banco de dados caso não exista e gerará as tabelas estruturadas de forma automática.
...
```sql
-- Criação da tabela de Pessoas/Autores
...
-- Criação da tabela de Registros de Progresso (registers)
CREATE TABLE IF NOT EXISTS `registers` (
  `id` VARCHAR(50) NOT NULL,
  `projeto` VARCHAR(255) NOT NULL,
  `atividade` VARCHAR(255) NOT NULL,
  `tipo_prazo` VARCHAR(20) NOT NULL, -- 'horas' ou 'data'
