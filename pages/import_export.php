        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col h-full">
                ...
                <p class="text-slate-600 dark:text-slate-300 mb-6">
                    Selecione um arquivo JSON exportado anteriormente para importar seus dados de volta para o sistema. 
                    <span class="text-amber-600 dark:text-amber-400 font-medium block mt-2">
                        <i class="fa-solid fa-triangle-exclamation mr-1"></i> Atenção: Isso pode criar duplicatas se os dados já existirem.
                    </span>
                </p>
        ...
        async function importData() {
            ...
            reader.onload = async (e) => {
                try {
                    ...
                    const result = await EffectaAPI.importData(data);

                    if (result.success) {
                        showStatus('Dados importados com sucesso!', 'success');
                        fileInput.value = '';
                    } else {
                        showStatus('Erro ao importar dados: ' + (result.error || 'Erro desconhecido'), 'error');
                    }
                } catch (error) {
